...
! routines for operating on it.
  ...
  type pump_type
	...
	real(DP) :: Enthalpy
	logical(kind=.TRUE.) :: StepFlows
	logical(kind=.TRUE.) :: OnDeliverability
  ...
  type ObsPoint_type
	...
	logical(kind=.TRUE.) :: IsPumpObsPoint  ! For pumps on deliverability
  ...
  logical(kind=.TRUE.) :: ErrorsKnown  
  logical(kind=.TRUE.) :: WellBlockIncl
...
!   Argument variables:
    real(DP) :: TestEndTime
	logical(kind=.TRUE.):: StepFlows,RunToSS
!   Local variables:
...
!   Argument variables:
	...
	logical(kind=.TRUE.),intent(inout):: ResetTimeStepSize
	logical(kind=.TRUE.),intent(in) :: StepFlows
	logical(kind=.TRUE.),intent(in) :: RunToSS
...
!   Argument Variables:
	...
	logical(kind=.TRUE.), intent(in)   :: ResetTimeStepSize
...
!   Argument Variables:
	...
	logical(kind=.TRUE.), intent(in) :: SearchForwards, ResetTimeStepSize
!   Locals:
    integer(I4B) :: Index, Increment
	logical(kind=.TRUE.) :: Found
