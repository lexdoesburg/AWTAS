...
contains
  ...
  function NumericalSolution1D(CR,COND,RHOR,AAA,&
    ...
    ! num_iterations = 0 ! Added for debugging
...
!            NewValue=(P(1) + P(2)) / 2 ! Checking linear interpolation
          case(2) ! Temperature:
...
subroutine SetupGrid(LayerThickness,ActionWellRadius)
  ...
  if (.NOT.WellBlockIncl) call CalculateSandfaceWeights ! Changed from not() to .NOT. to allow compilation
...
subroutine SetupBlockSizes(ActionWellRadius)
  ...
  !   Local variables:
  real(DP),parameter     :: ConstDR=0.01_dp ! Changed from 0.05 to 0.01 to match TOUGH2 data
  real(DP),parameter     :: GrowthFactor=1.2_dp
  integer(I4B),parameter :: NConstBlocks=20 ! Changed from 10 to 20 to match TOUGH2 data
  integer(I4B)           :: i
...
subroutine AssignBlockProperties(k,phi,SkinFactor,ActionWellRadius,RechargeCoef,Compressibility)
  ...
  if (ActionWellRadius > 0.0_dp) then ! May not be the best way of dealing with this (Lex)
