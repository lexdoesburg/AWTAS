...
class Model():
    ...
    def residual_function(self, parameters):
        ...
        if len(self.data.observation) != len(model_data):
            print('Model finished early - return large residual\n--------')
            self.chi_sq = 999999999999995
            return [99999999]*len(self.data.observation)
        else:
            self.chi_sq = self.__chi_squared(model_data)
            print('Current chi_squared = {}\nCurrent sum of residuals = {}\n--------'.format(self.chi_sq, np.sum(self.data.observation - model_data)))

        return self.data.observation - model_data

    def __chi_squared(self, approximation):
        if len(self.data.observation) == len(approximation):
            # sd = np.std(self.data.observation)
            # chi_squared = np.sum((self.data.observation-approximation)/sd)**2
            chi_squared = np.sum((self.data.observation-approximation))**2
        else:
            chi_squared = 9999999999999
        return chi_squared
    ...
    def find_model_parameters(self, phi=None, k=None, curve_fit=False, initial_parameters=None):
        ...
        if curve_fit:
            ...
            calls = 0
            # broken = False
            self.estimates = np.ndarray(shape=(5,225))
            i = 0
            for k in [1e-16, 1e-15, 1e-14, 1e-13, 1e-12]:
                for phi in [0.2, 0.15, 0.1, 0.05, 0.01]:
                    # initial_parameters = np.array([phi, k])
                    for p0 in np.linspace(90, 110, 3):
                        for x0 in np.linspace(0.15, 0.3, 3):
            # for k in [2e-15,3e-15]:
            #     for phi in [0.075, 0.085]:
            #         # initial_parameters = np.array([phi, k])
            #         for p0 in [100, 103]:
            #             for x0 in [0.2,0.25]:
                            initial_parameters = np.array([p0, x0, phi, k])
                            calls += 1
            ...
            # chi_squared = self.__chi_squared()
                            self.estimates[:, i] = optimal_parameters[0], optimal_parameters[1], optimal_parameters[2], optimal_parameters[3], self.chi_sq
            # print('Phi: {} k: {} Chi squared: {}'.format(phi, k, chi_squared))
                ...
                # if broken:
                #     break
            index = np.argmin(abs(self.estimates[4]))
            print('index = {} chi squared = {}'.format(index, self.estimates[4, index]))
            # print('Function called: {} times'.format(calls))

            optimal_parameters = self.estimates[:5, index]
            # phi, k = parameters
