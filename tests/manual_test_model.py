...
observation_points = data_class.ObservationPoints(radial_location=0.1, property='pressure', num_data=len(time))
...
pump_info = data_class.Pump(pumping_scheme='measured', flow_rates=pump_rates, flow_times=pump_times, injection_well=False, injection_enthalpy=0.0, deliverability=False, production_index=0.0, cutoff_pressure=0.0)
grid_info = data_class.Grid(num_blocks=100, num_constant_blocks=10, constant_block_size=0.1, block_growth_factor=1.2)

data = data_class.Data('radial1d', time=time, observation=expected_pressure, parameters_list=parameters, pump_info=pump_info, observation_points=observation_points, grid_info=grid_info)
...
modelled_pressure = radial1d_model.model(optimal_parameters)

plt.plot(data.time/86400, data.observation/1e5, 'k-', label='Observation')
plt.plot(data.time/86400, modelled_pressure/1e5, 'r--', label='Modelled value')
plt.show()
